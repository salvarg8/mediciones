...
public class FrmCalibracionSensor extends JDialog {
    ...
    private void initComponents() {
        ...
        titleAndSensor.add(titleLabel);
        titleAndSensor.add(sensorCombo);
        ...
        txtPresion1 = new JTextField("0.0");
        txtPresion1.setEditable(false);
        ...
        puntosPanel.add(btnConfirmar1);
        puntosPanel.add(new JLabel("Valor 2 (Ref):", SwingConstants.RIGHT));
        puntosPanel.add(txtPresion2);
        puntosPanel.add(btnConfirmar2);
        ...
        lblPresionActual.setFont(new Font("Consolas", Font.BOLD, 48));
        gbc.gridy = 2;
        gbc.weighty = 0.3;
        add(lblPresionActual, gbc);
        ...
        lblSignal0 = new JLabel("Voltaje en Punto 1: --- V");
        lblSignal0.setFont(new Font("Arial", Font.PLAIN, 16));
        lblSignal7 = new JLabel("Voltaje en Punto 2: --- V");
        lblSignal7.setFont(new Font("Arial", Font.PLAIN, 16));
        voltajePanel.add(lblSignal0);
        voltajePanel.add(lblSignal7);
        gbc.gridy = 3;
        gbc.weighty = 0.1;
        add(voltajePanel, gbc);
        ...
        lblA = new JLabel("a =", SwingConstants.RIGHT);
        lblA.setFont(new Font("Arial", Font.BOLD, 18));
        lblAValue = new JLabel("???");
        lblAValue.setFont(new Font("Arial", Font.BOLD, 18));
        lblC = new JLabel("c =", SwingConstants.RIGHT);
        lblC.setFont(new Font("Arial", Font.BOLD, 18));
        lblCValue = new JLabel("???");
        lblCValue.setFont(new Font("Arial", Font.BOLD, 18));
        labelsPanel.add(lblA);
        labelsPanel.add(lblAValue);
        labelsPanel.add(lblC);
        labelsPanel.add(lblCValue);
        ...
        constantesPanel.add(labelsPanel, BorderLayout.NORTH);
        constantesPanel.add(buttonsPanel, BorderLayout.CENTER);
        gbc.gridy = 4;
        gbc.weighty = 0.2;
        add(constantesPanel, gbc);

        JPanel portPanel = new JPanel(new FlowLayout(FlowLayout.CENTER, 20, 5));
        txtRawData = new JTextField(20);
        ...
        portPanel.add(new JLabel("RAW:"));
        portPanel.add(txtRawData);
        portPanel.add(new JLabel("Puerto:"));
        portPanel.add(portCombo);
        portPanel.add(new JLabel("Baud:"));
        portPanel.add(baudCombo);
        gbc.gridy = 5;
        gbc.weighty = 0.1;
        add(portPanel, gbc);
    }
    ...
    private void setupListeners() {
        ...
        btnConfirmar2.addActionListener(e -> {
            ...
            if (!selectedSensor.equals("LM35") && Double.isNaN(voltageAtZeroBar)) {
                JOptionPane.showMessageDialog(this, "Capture Punto 1 primero.", "Advertencia", JOptionPane.WARNING_MESSAGE);
                return;
            }
            if (!startPortIfNotOpen()) return;
            if (Double.isNaN(currentRawVoltage)) {
                JOptionPane.showMessageDialog(this, "Sin señal del sensor.", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }

            if (!ValidadorUI.esNumeroValido(txtPresion2, "Valor 2 (Ref)", this)) {
                return;
            }

            try {
                double val2 = Double.parseDouble(txtPresion2.getText().trim().replace(",", "."));
        ...
        btnAceptar.addActionListener(e -> {
            if (Double.isNaN(voltageAtKnownBar)) {
                JOptionPane.showMessageDialog(this, "Falta capturar Punto 2. Confirme el valor antes de guardar.", "Advertencia", JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            if (selectedSensor.equals("Motorola")) {
                ...
                calib.setA3(a3);
                calib.setC3(0.0);
            } // ⭐️ forzar c3 = 0 al guardar

            // Transformamos y guardamos seguro
            calib.setPresionConocida(Double.parseDouble(txtPresion2.getText().trim().replace(",", ".")));
            ...
            if (calibracionController.guardarCalibracion(calib)) {
                JOptionPane.showMessageDialog(this, "Calibración guardada exitosamente.", "Éxito", JOptionPane.INFORMATION_MESSAGE);
                dispose();
            }
        });
    ...
    private void loadCurrentCalibration() {
        try {
            ...
            if (e != null) {
                a2 = e.getA2() != null ? e.getA2() : 1.0;
                c2 = e.getC2() != null ? e.getC2() : 0.0;
            }
            SensorCalibracion l = calibracionController.obtenerUltimaCalibracionPorSensor("LM35");
            if (l != null) {
                a3 = l.getA3() != null ? l.getA3() : 1.0;
