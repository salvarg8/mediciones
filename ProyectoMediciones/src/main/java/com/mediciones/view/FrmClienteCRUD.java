...
public class FrmClienteCRUD extends JFrame {
    ...
    private void adjustLayout(double scaleFactor) {
        ...
        lblNit.setFont(scaledFont);
        txtNombre.setFont(scaledFont);
        txtNit.setFont(scaledFont);
    ...
    private void initComponents() {
        ...
        formBorder = BorderFactory.createTitledBorder("Nuevo/Editar Cliente");
        ...
        tableBorder = BorderFactory.createTitledBorder("Clientes Registrados");
    ...
    private void btnGuardarActionPerformed(ActionEvent e) {
        ...
        if (clienteSeleccionado == null) {
            clienteAGuardar = new Cliente(nombre, nit);
            mensajeExito = "Cliente guardado exitosamente.";
        } else {
            clienteAGuardar = clienteSeleccionado;
            clienteAGuardar.setNombre(nombre);
            clienteAGuardar.setNit(nit);
            mensajeExito = "Cliente actualizado exitosamente.";
        }

        try {
            boolean success = controller.guardarOActualizarCliente(clienteAGuardar);
            ...
            if (success) {
                ...
                JOptionPane.showMessageDialog(this, "Error al guardar/actualizar el cliente (Controller retornó false).", "Error", JOptionPane.ERROR_MESSAGE);
            }
        } catch (Exception ex) {
    ...
    private void btnEditarActionPerformed(ActionEvent e) {
        ...
        if (filaSeleccionada >= 0) {
            try {
                ...
                String nit = (String) tblClientes.getValueAt(filaSeleccionada, 2);

                clienteSeleccionado = new Cliente(idCliente, nombre, nit);
